...
char *req_server(const struct_adress * rss_server)
{
	if (rss_server != NULL) {
		...
		int addr_status, sock_status, connect_status, numbytes,
		    send_status;
		...
		if (argc != 2) {
			if (argc < 2)
				fprintf(stderr,
					"ERROR: ZU WENIGE PARAMETER; BENÖTIGT: DOMÄNE ODER IP\n");
			else
				fprintf(stderr,
					"ERROR: ZU VIELE PARAMETER; BENÖTIGT: DOMÄNE ODER IP\n");
			return NULL;
		}
#endif
		...
		inet_ntop(p->ai_family, get_in_addr(p->ai_addr), ipstr,
			  sizeof ipstr);
