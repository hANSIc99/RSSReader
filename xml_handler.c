...
int
dom_parser (const char *content, int length,
	    struct_news_list * list_ptr)
{
   ...
   */
  doc =
    xmlReadMemory (content, length, "noname.xml", NULL,
		   XML_PARSE_NOERROR /* 0 */ );
...
void
get_dom_objects (xmlNode * a_node,
		 struct_news_list * list_ptr)
{
  ...
  for (cur_node = a_node; cur_node;
      ...
      if (cur_node->type == XML_ELEMENT_NODE)
	{
	  ...
	  if (strncmp
	      ((char *) cur_node->name, description_name,
	       description_size) == 0)
	    {
		  ...
		  if (cur_node->children != NULL)
		      ...
		      else
			{
			  ...
			  strncpy (clean_string,
				   (char *)
				   cur_node->children->
				   content, string_lenght);
			  clean_string[string_lenght] =
			    '\0';
			}
...
load_data (char *xml_string)
{
  ...
  temp =
    get_temp_string (xml_string, startzeichen, str_lenght);
  ...
  if ((dom_parser
       (rss_string, (strlen (rss_string)), lists)) == 0)
    {
...
char *
get_temp_string (char *xml_string, int startzeichen,
		 int str_lenght)
{
  ...
  temp3 =
    xml_string + (size_t) ((startzeichen) / sizeof (char));
...
char *
get_server_info (char *xml_string, int startzeichen)
{
...
char *
get_rss_tag (char *temp_string, char *end_tag,
...
int
get_starttag (const char *xml_string, const char *start_tag)
{
